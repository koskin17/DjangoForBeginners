...
urlpatterns = [
    ...
    path('products/', include('products.urls', namespace='products')), # Также тут примененно пространство имён и все маршруты с products теперь имеют пространство имён "products" и обрабатываются приложением products
    path('users/', include('users.urls', namespace='users')),  # При переходе на наш сайт / проект будет открываться страница с пользователями, которую мы описали как users и которая вызывается функцией users из файла users/views.py.
